...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        cur_len = input_ids.shape[-1] - 1## 减去开头的eos token
        batch_size = len(self._beam_hyps) ##
        assert batch_size == (input_ids.shape[0] // self.group_size)

        device = input_ids.device
        ## group_size 在beam group为1的时候就是beam size
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            #self._done 是一个长度为batch的bool向量， 记载每个batch是否完成搜索 如果完成就在对应batch序列后面padding
...
class BeamHypotheses:
    ...
    def add(self, hyp: torch.LongTensor, sum_logprobs: float):
        ...
        score = sum_logprobs / (hyp[hyp!=2].shape[-1] ** self.length_penalty)
